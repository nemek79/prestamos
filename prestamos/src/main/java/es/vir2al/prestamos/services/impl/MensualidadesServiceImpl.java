...
public class MensualidadesServiceImpl implements MensualidadesService {
	...
	public void setMensualidadActual(PrestamoDTO prestamo) throws Exception {
		...
		// si el prestamo está en su mes inicial y se paga mensualidad, es la del mes
		// siguiente
	...
	public EstadoMensualidadDTO getEstadoMensualidad(PrestamoDTO prestamo, Integer mes, Integer year) throws Exception {
		...
		Mensualidad mensualidadBD = this.mesualidadesDAO.findByPrestamoAndMesAndYear(prestamo.asPrestamo(), mes, year)
				.orElse(null);
		...
		if (mensualidadBD != null) {
			...
			if (this.isMensualidadInMesCreacion(prestamo.getFechaIni())) {
				...
				mensualidadBD = this.mesualidadesDAO
						.findByPrestamoAndMesAndYear(prestamo.asPrestamo(), mesSiguiente, yearAbierto).orElse(null);
	...
	/**
	 * Comprueba si la fecha del pago de la mensualidad ya ha expirado y va con
	 * retraso dentro del mes actual
	 * 
	 * @param fechaIn
	 * @return
	 * @throws Exception
	...
	private boolean isFechaPagoExpired(String fechaIn) throws Exception {
		...
		// Estamos en el mes y año correctos, comprobar si la fecha actual es anterior a
		// la de pago
		Date hoy = new Date();
	...
	/**
	 * Comprueba si el prestamo está en el mes de apertura En tal caso, la
	 * mensualidad está abierta, en este mes no se paga mensualidad
	 * 
	 * @param fechaInicio
	 * @return
	 * @throws Exception
	...
	public MensualidadDTO getActualByPrestamo(PrestamoDTO prestamo) throws Exception {
		...
		Mensualidad mensualidadBD = this.mesualidadesDAO.findByPrestamoAndMesAndYear(prestamo.asPrestamo(),
				Utilidades.getMesActual(), Utilidades.getYearActual()).orElse(null);
	...
	public Map<Integer, Float> getInteresesTotalesByMesFromYear(Integer year) throws Exception {
		...
		Float totalYear = 0f;
		int index = 1;
		MensualidadByMonthsDTO mes = null;

		List<MensualidadByMonthsDTO> lstResultadosPorMes = this.mesualidadesDAO.getInteresByMesFromYear(year);
		Iterator<MensualidadByMonthsDTO> it = lstResultadosPorMes.iterator();
		...
		while (it.hasNext()) {
			...
			while (mes.getMes() > index) {
				data.put(index,0f);
				index++;
			}

			if (index < 13) {
				data.put(mes.getMes(), mes.getIntereses());
				totalYear += mes.getIntereses();
				index++;
			}
